# -*- coding: utf-8 -*-
"""
Created on Wed Jul 17 12:25:15 2019
...
def archives(path_general,obj):#,verbose=False):
    path=path_general+'/LCO_images/raw/%s/'%obj
    ...
    if not os.path.exists(path_general+'/archive_images/%s'%obj):
        os.mkdir(path_general+'/archive_images/%s'%obj)
    #else:
    ...
    if xid!=None:
        ...
        if os.path.exists(path_general+'/archive_images/%s/%s_SDSS.fits'%(obj,obj)):
            os.remove(path_general+'/archive_images/%s/%s_SDSS.fits'%(obj,obj))
        fits.HDUList.writeto(im[0],path_general+'/archive_images/%s/%s_SDSS.fits'%(obj,obj),overwrite=True)
    ...
    if len(paths)!=0:
        for i in range(len(paths)):
            fits.HDUList.writeto(paths[i],path_general+'/archive_images/%s/%s_%s.fits'%(obj,obj,survey[i]),overwrite=True)
    
    #if no archive images program exits
    if xid==None and len(paths)==0:
        
        for filename in sorted(os.listdir(path_general+'/LCO_images/raw/')):
            if filename=='.DS_Store': #problematic folder that shows up sometimes
                print('.')
            else:
                n=len(sorted(os.listdir(path_general+'/archive_images/%s'%filename)))
                dest=path_general+'/rejects'
                if n==0:
                    if os.path.exists(dest+'/'+filename)==True:
                        shutil.rmtree(dest+'/'+filename,ignore_errors=True)
                    shutil.move(path_general+'/LCO_images/raw/%s'%filename,dest)
